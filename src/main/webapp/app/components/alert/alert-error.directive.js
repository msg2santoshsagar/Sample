(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope, $translate) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('sampleApp.httpError', function (event, httpResponse) {
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-sampleApp-error');
                var entityKey = httpResponse.headers('X-sampleApp-params');
                if (errorHeader) {
                    ...
                    for (i = 0; i < httpResponse.data.fieldErrors.length; i++) {
                        ...
                        var fieldName = $translate.instant('sampleApp.' + fieldError.objectName + '.' + convertedField);
